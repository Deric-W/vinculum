...
impl<M, C> FossilCollection<M, C> {
    ...
    pub fn from_parts<F, S>(fossils: F, seen_manifests: S, timestamp: std::time::SystemTime) -> FossilCollection<M, C>
    where
...
impl<M, C, E> FossilCollectionError<M, C, E> {
    ...
    /// Consumees this error, returning the underlying error and
...
impl<M, C> FossilCollectionBuilder<M, C> {
    ...
    ///
    /// This method turns all fossil canidates into fossils and stores them
    ...
    C: Hash + Eq,
{
    ...
    ///
    /// This can be used to avoid storing unecessary data in memory since manifests
    ...
    C: Hash + Eq,
{
    ...
    where
        ...
        <<R as Repository<M, I, C>>::Manifest as Manifest<I, C>>::ReferencedChunks: Unpin,
    {
        <Self as FossilDeleter<M, I, C>>::delete(
            self,
            repository,
            parallelism,
        )
    ...
    C: Eq + Hash,
{
    ...
    /// but checks whether the fossil canidate was already added as a referenced chunk,
    ...
    ///
    /// This method removes matching fossil canidates.
