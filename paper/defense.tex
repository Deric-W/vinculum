...
\begin{document}
    ...
    \begin{frame}
        ...
        \begin{center}
            \includesvg[width=.9\textwidth, height=.9\textheight, pretex=\small]{terminology.drawio.svg}
        \end{center}
    \end{frame}
    ...
    \begin{frame}
        \frametitle{Example}
        \begin{itemize}
            \item the dyed chunks are unreferenced and are therefore pruned
            \item the red chunk is referenced by a manifest in the process of being created
        \end{itemize}
        \begin{center}
            \includesvg[height=.6\textheight, pretex=\small]{lock-free_deduplication_example.drawio.svg}
        \end{center}
    \end{frame}
    ...
    \begin{frame}
        ...
        \begin{itemize}
            ...
            \item \textit{FossilCollection} struct: stores data required by the
                fossil deletion step and provides methods to perform it
            ...
            \item \textit{PipelinedFossilCollectionBuilder} struct: allows reuse data
                acquired during a fossil deletion step for a fossil collection step
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}
        \frametitle{Setup}
        \begin{itemize}
            \item a simple repository was implemented on top a UNIX file system
                together with a command line application
            \item three existing backup programs with a similar internal architecture
    ...
    \begin{frame}
        ...
        \begin{itemize}
            \item three VM images were backed up with a fixed chunk size of 1 MiB
            \begin{enumerate}
            ...
            \end{enumerate}
            \item they were deleted in FIFO order
            \item all programs performed the deletion with two operations which were measured independently
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}
        ...
        \begin{itemize}
            ...
            \item the sum of the times required by the two operations required for deletion is displayed
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}
        ...
        \begin{itemize}
            \item a fixed number of chunks with a size of 4096 bytes were created
