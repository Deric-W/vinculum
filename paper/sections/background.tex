...
\section{Current developments and challenges}
...
Examples of this approach are Déjà Dup~\cite{DejaDup} and Duplicity~\cite{Duplicity} on which it is based.
...
Examples of this approach are Pika Backup~\cite{Pika} and Borg 1~\cite{Borg1} on which it is based,
Borg 2~\cite{Borg2}, restic~\cite{restic}, Duplicacy~\cite{Duplicacy}, Duplicati~\cite{Duplicati}
and Kopia~\cite{Kopia}.
...
\begin{samepage}
    \paragraph{Policy 1} (Checking before Uploading).
    \textit{
...
repository will find any manifest files which may still reference the fossils.
...
\begin{enumerate}
    \item A fossil collection step reverts a fossil recovery by turning the chunk back into a fossil
        ...
        after the referencing manifest files seen by the fossil deletion step where
    ...
    \item A fossil deletion step deletes a fossil
        ...
        deletion step can only see fossils created by fossil collection steps
        which completed before its associated fossil collection step and therefore
        any manifest file still referencing the fossil will be seen by the
