...
\Cref{tab:approaches} compares a selection of commonly used approaches.

A common scenario is the creation of periodic backups of a single dataset, for
example photos stored on a personal computer.
Because such datasets often do not change significantly between them individual
backups may contain redundant data which can be removed.
Such a deduplication of data can also be useful when multiple similar datasets
have to be backed up, for example all workstation machines in an office.

Another use case is the transformation of the generated backup data before storage.
Examples of this are applying compression to reduce the required storage space
or encrypting the data to protect it from unwanted access.

There is also the problem of the storage space available for backups being limited.
To allow continuous backups old backups have to be deleted which is controlled
by a backup retention policy.
...
\begin{table}
    \resizebox{\textwidth}{!}{
        \begin{tabular}{ c c c c }
            \toprule
            \textbf{Approach} & Hard links & Full/incremental backups & Content-addressable storage \\
            \midrule
            \textbf{Deduplication} & individual files & changed data & chunks \\
            \midrule
            \textbf{Transformations} & no & yes & yes \\
            \midrule
            \textbf{Constraints on deletion} & no & yes & no \\
            \bottomrule
        \end{tabular}
    }
    \caption{Comparison of a selection of existing backup approaches.}\label{tab:approaches}
\end{table}

\paragraph{Hard links} A possible way of implementing such a solution is based on the
...
Backups are stored in separate directories and creating a new one consists of
copying the data to be backed up into a new directory while instructing rsync
to search for identical files in the directories containing existing backups.
This in turn allows saving storage space by sharing files contained in a specific
backup with other backups containing the same files.
...
`a fast, versatile, remote (and local) file-copying tool'~\cite{rsync} instead
of a pure backup solution.
Another advantage is the ability to detect and store differences between files of different
backups, allowing to save storage space where rsync would require creating a new
...
by sharing the chunks containing it~\cite[section `Internals']{Borg1}.
...
Besides the aforementioned solutions there are a number of
alternatives like Bacula~\cite{Bacula} which because of their comparatively low
popularity and time constraints where not further investigated.

\section{Lock-Free Deduplication}

\subsection{Duplicacy}

In 2020 Gilbert et al.\ proposed~\cite{Duplicacy} a solution to
...
objects contain which chunks and the associated problem of parallel
...
section but when uploading them to the repository obeys the following policy~\cite{Duplicacy}:
...
\begin{figure}
    \includesvg[width=0.95\textwidth, pretex=\small]{naive_deletion.drawio.svg}
    \caption{Example where directly deleting chunks causes a chunk to be deleted prematurely.}\label{fig:naive-deletion}
\end{figure}
...
fail since the chunk does no longer exist in the repository.
An example of such a scenario is depicted in \Cref{fig:naive-deletion} where
\nameref{para:policy1} causes the client creating the manifest file to reference
an existing chunk which is deleted before the manifest file referencing it can
be seen by the client deleting unreferenced chunks.
To prevent this instead of being deleted such chunks are first turned into `\textit{fossils}'
by renaming them, allowing them to be referenced by the following policy~\cite{Duplicacy}:
...
which are in the process of being created as by the following policy~\cite{Duplicacy}:
...
or fossil collection for short, which was created by the fossil collection step
and stored outside the repository locally by the client performing these
operations~\cite[section `Two Step Fossil Deletion']{Duplicacy}. 
...
In the example there are 3 clients which operate on a central repository.
...
the other manifests files reference it.
...
collection steps.
This in turn can cause it to delete them even before \nameref{para:policy3} would allow the
fossil deletion step associated with the fossil collection step that created them
to be executed, violating the assumption that the fossil will no longer be referenced
by manifest files which are in the process of being created.

In the example this is exploited by a client creating a manifest file called `\#3` referencing
the chunk which was looked up before being turned into a fossil and therefore not re-uploaded
again.
Should this manifest file be created after the fossil deletion step downloaded
all manifest files the fossil will be deleted and therefore cause the restoration
of the associated backup to fail.
...
\begin{samepage}
    \begin{thm}\label{thm:singleclient}
        ...
        By limiting the fossil collection and deletion steps to a single client and the
...
\begin{enumerate}
    \item A fossil collection step reverts a fossil recovery by turning the chunk back into a fossil
        ...
        Since the fossil deletion step has to be completed before the fossil collection
        step can be started any remaining manifest files seen by it are also seen by the
        fossil collection step.
    ...
    \item A fossil deletion step deletes a fossil
        ...
        The fossil deletion step can only receive fossils from its associated
        fossil collection step and only start after \nameref{para:policy3}
        ensures no new manifest files can reference fossils created before its
        associated fossil collection step ended.
