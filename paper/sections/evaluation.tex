...
in chunks like how it is done by Duplicacy~\cite[section `Naming Chunks']{Duplicacy}
but is instead stored directly in the files using a simple binary encoding.
...
\begin{table}
    ...
    \begin{tabular}{ c c c c c }
        \toprule
        \textbf{Program} & Borg 1 & Borg 2 & Duplicacy & vinculum-benchmark \\
        \midrule
        \textbf{Version} & 1.4.0  & 2.0.0b14 & 3.2.4 & 0.1.0 \\
        \midrule
        \textbf{Programming language} & Python, C & Python, C & Go & Rust \\
        \bottomrule
    \end{tabular}
...
of Gilbert et al.~\cite{Duplicacy}.
...
selected as developer tools, causing the images to be around 9 GB in size.
\begin{itemize}
    \item clang
    \item rust-all
    \item git
...
After creating the backups with a fixed chunk size of one MiB to ensure every
program generating the same chunks they were pruned in the order of their creation.
...
between the start of the commands and their termination.
...
deletion operations with the last removal taking the longest, followed by the first
and second removal, which can be explained by the amount of data deleted by each
operation.
...
during backup creation and deletion~\cite[section `Internals']{Borg1}, replacing
...
having to manage a smaller number of files, turning the segment files into a
disadvantage.
...
a quadratic growth the runtime of Duplicacy and Vinculum seems to be constant.
...
may be explained by the caching operations performed by Duplicacy as described
