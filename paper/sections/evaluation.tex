...
\section{Virtual machine images}\label{sec:vm-images}
...
\begin{figure}
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Time required to delete individual backups, without Borg 1.}\label{fig:vm-images-zoomed}
\end{figure}
...
To better show their differences \Cref{fig:vm-images-zoomed} shows them without
the results generated by Borg 1.
...
\foreach \f in {64, 4096, 32768, 524288}{
    \ifnum\f=4096
        \pgfplotstableread[col sep = comma]{benchmark/results/chunks\f large.csv}{\chunkdata}
    \else
    ...
    \begin{figure}
        \begin{tikzpicture}
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Time required to delete a single backup, chunk size = \f{} bytes.}\label{fig:chunks\f}
    \end{figure}
}
...
depicted in \Cref{fig:chunks4096}.

While Borg 1 does not exhibit a significantly increased runtime when using small
chunk sizes its performance degrades when increasing them.
...
in \Cref{sec:vm-images} will also increase and seems to surpass the advantage of
...
\foreach \f in {0, 1, 1000}{
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Time required to delete a single backup, total chunks = \f{}.}\label{fig:backups\f}
    \end{figure}
}

\begin{figure}
    \pgfplotstableread[col sep = comma]{benchmark/results/manifests1const.csv}{\manifestdata}
    \begin{tikzpicture}
        \pgfplotsset{
            width=.9\textwidth,
            height=.4\textheight,
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Time required to delete a single backup, total chunks = 1 without Borg 1 and 2.}\label{fig:backups-zoomed}
\end{figure}

In contrast to my expectations the results do not seem to be affected by the
total number of chunks existing in the repository with a possible explanation
being the number of chunks being too low to have a visible effect.
...
When looking at the magnified data presented in \Cref{fig:backups-zoomed} it
becomes apparent that their runtime also increases with the number of backups,
