...
level playing field they represent synthetic scenarios and the vinculum-benchmark
program described in \Cref{sec:repo-implementation} provides only primitive options
for creating backups while not dealing with concerns such as privacy or reducing
possible network access through caching, therefore being unusable as a real backup
solution.
...
Since implementing real-world solutions requires the consideration of additional
concerns the users have to implement features such as encryption, compression and
...
failure.
While \Cref{sec:errors} provides methods to recover from clients experiencing
