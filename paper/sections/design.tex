...
the operations performed by clients on the repository it must be assumed they are atomic.
This section will examine the feasibility of this assumption when using a POSIX~\cite{Posix}
file system.

\subsection{Creating Chunks and Manifest Files}\label{sec:creation}
...
To prevent clients from opening the same file in the directory for temporary files
...
`unlink'~\cite{Posix} function which is also available on all POSIX conforming systems.
...
happening during its execution would be equivalent to them being moved freely before or
...
directory enumeration operation an atomic operation can not guarantee
...
is no difference between them in this regard.
...
deletion steps can rename the chunk or its fossils between their executions.
...
\nameref{para:policy3} is satisfied before continuing, causing the fossil deletion
steps associated with fossil collection steps which completed after the client created
its last manifest file to be delayed until the client creates a new manifest file.
Since \Cref{thm:singleclient} limits the amount of pending fossil deletion steps
to one the chunk can only be recovered once during the access operations performed
by the client.

Should the client fail to access either the chunk or its associated fossil on
the first try because the fossil was recovered it will succeed in accessing one
of them on the second try because the fossil can not be recovered a second time.
...
files~\cite[section `DESCRIPTION' of the `unlink' and `rename' functions]{Posix}
this situation requires no further mitigations.

\section{Error Recovery}\label{sec:errors}
...
Should a client be interrupted while creating a backup he might have already
uploaded some chunks to the repository without creating a manifest file.
...
parallel and therefore have to be made visible to a fossil collection step to
allow them to be deleted by the Lock-Free Deduplication algorithm.
...
in the repository (and where therefore created) before it completed \nameref{para:policy3}
...
Both of those problems could be solved by allowing clients to be added and removed
from the repository.
This section investigates which conditions have to be upheld to not disrupt
the Lock-Free Deduplication algorithm.

\subsection{Adding Clients}\label{sec:adding-clients}
...
In addition to creating manifest files a new client can interact with a fossil
deletion step by it being contained in this list.
...
already exist in the repository since a fossil deletion step implementing
\Cref{thm:singleclient} will only access fossils created before its associated
fossil collection step completed.
Since the new manifest files of the client can not reference the existing fossils
because of \nameref{para:policy2} the fossil deletion step does not have to be
delayed until their creation to correctly determine whether a fossil has to be
recovered.
...
\subsection{Removing Clients}\label{sec:removing-clients}
...
before he was removed from the repository and are therefore visible to the
...
associated fossil deletion step before being removed the fossils created by it
will be abandoned and can be removed by the method described in \Cref{sec:abandoned-fossils}.
